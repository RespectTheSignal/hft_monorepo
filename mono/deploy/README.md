...
│   ├── prometheus/
│   │   └── prometheus.yml       ← publisher / gateway / strategy scrape config
│   └── grafana/
│       ├── provisioning/        ← datasource / dashboard provider auto-provision
│       └── dashboards/          ← HFT Pipeline dashboard JSON
├── scripts/
...
Prometheus + Grafana 배포 예시는 `deploy/monitoring/` 아래에 있다.
...
- Prometheus: `9090`
- Grafana: `3000`
- publisher `/metrics`: `9100`
- order-gateway `/metrics`: `9101`
- strategy `/metrics`: `9102`

`prometheus/prometheus.yml` 의 `infra-vm`, `strategy-vm-0` 는 **placeholder host명**이다.
실운영에서는 DNS 또는 고정 IP 로 교체해야 한다.

Grafana datasource 는 두 개를 provision 한다.

- `Prometheus` → `http://prometheus:9090`
- `QuestDB` → `questdb:8812`

QuestDB 가 monitoring compose 와 같은 Docker network 에 없으면,
`grafana/provisioning/datasources/datasources.yml` 의 `questdb:8812` 를 실제 접근 가능한
host:port 로 바꿔야 한다.

대시보드 `grafana/dashboards/hft-pipeline.json` 은 다음 5개 row 를 제공한다.
