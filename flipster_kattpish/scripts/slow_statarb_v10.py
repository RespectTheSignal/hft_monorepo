#!/usr/bin/env python3
"""Slow Stat-Arb v10 — Rust strategy.rs port (Python).

Architectural mirror of strategy_flipster_python/src/strategy.rs:

  • DataCache — per-symbol BookticketSnapshot history at 1s/5s/10s/20s
  • SignalLoop — fan-out per symbol (concurrent tasks via asyncio.gather)
  • OrderQueue + Workers — non-blocking order placement (asyncio.Queue + N workers)
  • RestartManager — strategy state reset every restart_interval_secs (default 600s)
  • LatencyGuard — reject if gate_bt > 200ms or base_bt > 300ms
  • MarketWatcher — rolling avg_mid_gap + avg_spread (60s window)
  • OrderCountTracker — per-symbol rate limit (Rust: increment_order_count)
  • AccountHealth — reject if margin_avail < threshold
  • decide_order — multi-confirmation across 1s/5s/10s/20s timeframes

Reference exchange (Rust calls "base"): Binance perp.
Web/secondary reference (Rust calls "gate_web"): Gate perp + Bitget perp consensus.
Trading exchange (Rust calls "gate"): Flipster perp.
"""
...
def _load_symbols_cfg() -> dict:
    ...
    if not cfg_path.exists():
        return {"BTC": ("btcusdt", "BTC_USDT", "BTCUSDT", "BTCUSDT.PERP", 0.1)}
    data = json.loads(cfg_path.read_text())
    cfg = {}
    for d in data:
        base = d["base"]
        cfg[base] = (
            f"{base.lower()}usdt",                # binance stream name
            f"{base}_USDT",                       # gate sym
            f"{base}USDT",                        # bitget sym
            f"{base}USDT.PERP",                   # flipster sym
            d.get("tick", 0.0001),
            d.get("binance_vol_usd", 0.0),
        )
    return cfg
...
class BotState:
    ...
    log_path: Path = field(default_factory=lambda: Path("logs/slow_statarb_v10.jsonl"))
    ...
    if s.state == "IDLE":
        ...
        try:
            state.order_q.put_nowait(OrderRequest(
                ...
                amount_usd=cfg.amount_usd, leverage=cfg.leverage,
