...
// Window in minutes; overridable via HSR_GAP_WINDOW_MIN env (default 40
// — backtest sweep showed 35-45 is the sweet spot, +21 bp / 95% win).
fn gap_window_min() -> i64 {
    std::env::var("HSR_GAP_WINDOW_MIN")
        .ok()
        .and_then(|s| s.parse().ok())
        .filter(|v: &i64| *v > 0 && *v <= 240)
        .unwrap_or(40)
}
