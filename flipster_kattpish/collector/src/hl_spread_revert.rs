...
//!   strategy = "hl_spread_revert"
//!   account_id = "HL_SPREAD_REVERT_v1" (or SR_ACCOUNT_ID env)
...
use crate::hl_baseline_writer::{BaselinePoint, SharedBaselines};
use dashmap::DashMap;
use hl_pairs;
use once_cell::sync::Lazy;

/// Per-position pending maker-exit context. Set after entry IOC fills,
/// removed when paper signals close (cancel + fallback).
#[derive(Debug, Clone)]
struct PendingExit {
    coin: String,
    exit_oid: u64,
    target_px: f64,
    sz: f64,
    is_close_buy: bool, // direction of the close order (opposite of entry)
}

/// Map pos_id → pending maker exit. Shared across spawned tasks.
static PENDING_EXITS: Lazy<DashMap<u64, PendingExit>> = Lazy::new(DashMap::new);

/// Fire-and-forget IOC order via HL SDK. Doesn't block paper logic.
fn fire_live_order(
    le: Arc<hl_pairs::exec::LiveExec>,
    coin: String,
    side: &'static str,    // paper "long" / "short" — desired DIRECTION
    ref_mid: f64,
    variant_size_usd: f64,
    is_close: bool,
    tag: &'static str,     // "ENTRY" or "EXIT"
    account_id: &str,
) {
    let is_buy = match (side, is_close) {
        ("long",  false) => true,   // long entry → BUY
        ("short", false) => false,  // short entry → SELL
        ("long",  true)  => false,  // close long → SELL (reduce_only)
        ("short", true)  => true,   // close short → BUY (reduce_only)
        _ => return,
    };
    let cushion = 0.005;
    let limit_px = if is_buy { ref_mid * (1.0 + cushion) } else { ref_mid * (1.0 - cushion) };
    let limit_px = (limit_px * 10000.0).round() / 10000.0;
    let live_size_usd = variant_size_usd.min(le.max_size_usd());
    let raw_sz = live_size_usd / ref_mid;
    let sz = match le.round_size(&coin, raw_sz) {
        Some(s) if s > 0.0 => s,
        _ => return,
    };
    if sz * ref_mid < 11.0 { return; }  // HL min $10 + cushion
    let id = account_id.to_string();
    tokio::spawn(async move {
        match le.place_ioc(&coin, is_buy, limit_px, sz, /*reduce_only=*/is_close).await {
            Ok((fpx, fsz)) => info!(
                "LIVE {} {} {} {} sz={:.5} fill_px={:.4}",
...
/// Default mean-reversion target in bp for the maker exit limit. The exit
/// limit is placed at `entry_px * (1 ± target_bp/1e4 * direction)`. Set
/// via HSR_LIVE_TP_BP env. Default 8 bp — Flipster spread_revert backtests
/// show median reverse-exit gain around +14 bp gross, so 8 bp net target
/// gives a comfortable maker fill probability.
fn live_tp_bp() -> f64 {
    std::env::var("HSR_LIVE_TP_BP")
        .ok().and_then(|s| s.parse().ok())
        .filter(|v: &f64| *v > 0.0 && *v < 100.0)
        .unwrap_or(8.0)
}

/// Fire entry IOC + immediately place a maker exit limit at the
/// mean-reversion target. Saves pending exit oid for later cancel.
/// gate_lead executor pattern, ported.
///
/// SAFETY: if `live_state` is provided and the coin is already busy at
/// master level (active position OR resting order), this skips entry.
/// This is the "1 position per coin enforced at master level" guard.
fn fire_live_entry_with_maker_exit(
    le: Arc<hl_pairs::exec::LiveExec>,
    coin: String,
    side: &'static str,    // paper "long" / "short"
    ref_mid: f64,
    variant_size_usd: f64,
    pos_id: u64,
    account_id: &str,
    live_state: &Option<hl_pairs::live_state::LiveState>,
) {
    // Master-level guard: skip if coin already has position or resting order.
    if let Some(ls) = live_state {
        if ls.coin_busy(&coin) {
            debug!("LIVE {} ENTRY {} skip — coin busy at master (pos={:.4} or resting orders)",
                account_id, coin, ls.position_size(&coin));
            return;
        }
    }
    let entry_buy = side == "long";
    // 50bp slippage cushion on entry IOC.
    let entry_limit_px = if entry_buy { ref_mid * 1.005 } else { ref_mid * 0.995 };
    let entry_limit_px = (entry_limit_px * 10000.0).round() / 10000.0;
    let live_size_usd = variant_size_usd.min(le.max_size_usd());
    let raw_sz = live_size_usd / ref_mid;
    let sz = match le.round_size(&coin, raw_sz) {
        Some(s) if s > 0.0 => s,
    ...
    if sz * ref_mid < 11.0 { return; }
    let id = account_id.to_string();
    let tp_bp = live_tp_bp();
    tokio::spawn(async move {
        // 1) Entry IOC (taker, immediate fill)
        let (entry_fill_px, entry_fill_sz) = match le.place_ioc(&coin, entry_buy, entry_limit_px, sz, false).await {
            Ok(x) => x,
            Err(e) => { warn!("LIVE {} ENTRY {} FAIL: {}", id, coin, e); return; }
        };
        info!(
            "LIVE {} ENTRY {} {} sz={:.5} fill_px={:.4} (placing maker exit @ TP {}bp)",
            id, coin, if entry_buy {"BUY"} else {"SELL"}, entry_fill_sz, entry_fill_px, tp_bp
        );
        // 2) Maker LIMIT exit at TP target — opposite direction, reduce_only.
        //    long entry → SELL above; short entry → BUY below.
        let close_buy = !entry_buy;
        let target_px = if entry_buy {
            entry_fill_px * (1.0 + tp_bp / 1e4)  // long: SELL above entry
        } else {
            entry_fill_px * (1.0 - tp_bp / 1e4)  // short: BUY below entry
        };
        let target_px = (target_px * 10000.0).round() / 10000.0;
        match le.place_alo(&coin, close_buy, target_px, entry_fill_sz, /*reduce_only=*/true).await {
            Ok(oid) => {
                info!(
                    "LIVE {} MAKER-EXIT {} {} oid={} px={:.4} (entry fill {:.4})",
                    id, coin, if close_buy {"BUY"} else {"SELL"}, oid, target_px, entry_fill_px
                );
                PENDING_EXITS.insert(pos_id, PendingExit {
                    coin, exit_oid: oid, target_px, sz: entry_fill_sz, is_close_buy: close_buy,
                });
            }
            Err(e) => warn!("LIVE {} MAKER-EXIT {} place FAIL: {}", id, coin, e),
        }
    });
}

/// On paper-signal close: cancel pending maker exit. If cancel succeeds
/// the maker is unfilled → IOC fallback. If cancel fails ("ORDER_NOT_FOUND")
/// the maker already filled → nothing to do, master is flat.
fn fire_live_close(
    le: Arc<hl_pairs::exec::LiveExec>,
    pos_id: u64,
    fallback_ref_px: f64,
    ...
    let id = account_id.to_string();
    let pending = match PENDING_EXITS.remove(&pos_id) {
        Some((_, p)) => p,
        None => return,  // never had a pending live exit
    };
    tokio::spawn(async move {
        match le.cancel(&pending.coin, pending.exit_oid).await {
            Ok(()) => {
                // Maker was still resting → not filled. Force-close via IOC.
                let cushion = 0.005;
                let limit_px = if pending.is_close_buy {
                    fallback_ref_px * (1.0 + cushion)
                } else {
                    fallback_ref_px * (1.0 - cushion)
                };
                let limit_px = (limit_px * 10000.0).round() / 10000.0;
                match le.place_ioc(&pending.coin, pending.is_close_buy, limit_px, pending.sz, /*reduce_only=*/true).await {
                    Ok((fpx, fsz)) => info!(
                        "LIVE {} EXIT-IOC {} {} sz={:.5} fill_px={:.4} (maker cancel ok, market fallback)",
                        id, pending.coin, if pending.is_close_buy {"BUY"} else {"SELL"}, fsz, fpx
                    ),
                    Err(e) => warn!("LIVE {} EXIT-IOC {} FAIL: {}", id, pending.coin, e),
                }
            }
            Err(e) => {
                let msg = format!("{e}");
                ...
                {
                    info!(
                        "LIVE {} MAKER-EXIT {} FILLED @ {:.4} (oid {} cancel returned not-found)",
                        id, pending.coin, pending.target_px, pending.exit_oid
                    );
                } else {
                    warn!("LIVE {} MAKER-EXIT {} cancel ambiguous err: {}", id, pending.coin, e);
                }
            }
        }
    });
...
impl Default for SpreadRevertParams {
    fn default() -> Self {
        Self {
            account_id: "HL_SPREAD_REVERT_v1".to_string(),
            entry_size_usd: 10.0,
...
impl SpreadRevertParams {
    pub fn from_env() -> Self {
        ...
        if let Ok(v) = std::env::var("HSR_ACCOUNT_ID") {
            p.account_id = v;
        }
        if let Some(v) = std::env::var("HSR_SIZE_USD")
            .ok()
            .and_then(|s| s.parse().ok())
        {
            p.entry_size_usd = v;
        }
        if let Some(v) = std::env::var("HSR_MAX_POSITIONS")
            .ok()
            .and_then(|s| s.parse().ok())
        {
            p.max_open_positions = v;
        }
        if let Some(v) = std::env::var("HSR_MAX_POSITIONS_PER_BASE")
            .ok()
            .and_then(|s| s.parse().ok())
        {
            p.max_positions_per_base = v;
        }
        if let Some(v) = std::env::var("HSR_ENTRY_BP")
            .ok()
            .and_then(|s| s.parse().ok())
        {
            p.entry_threshold_bp = v;
        }
        if let Some(v) = std::env::var("HSR_STOP_BP")
            .ok()
            .and_then(|s| s.parse().ok())
        {
            p.stop_bp = v;
        }
        if let Some(v) = std::env::var("HSR_MAX_HOLD_S")
            .ok()
            .and_then(|s| s.parse().ok())
        {
            p.max_hold_s = v;
        }
        if let Some(v) = std::env::var("HSR_MIN_HOLD_MS")
            .ok()
            .and_then(|s| s.parse().ok())
        {
            p.min_hold_ms = v;
        }
        if let Some(v) = std::env::var("HSR_ENTRY_COOLDOWN_MS")
            .ok()
            .and_then(|s| s.parse().ok())
        {
            p.entry_cooldown_ms = v;
        }
        if let Some(v) = std::env::var("HSR_FEE_BP").ok().and_then(|s| s.parse().ok()) {
            p.flipster_fee_bp = v;
        }
        if let Ok(v) = std::env::var("HSR_WHITELIST") {
            p.whitelist = v
                .split(',')
                .filter(|s| !s.is_empty())
        ...
        if let Ok(v) = std::env::var("HSR_BLACKLIST") {
            p.blacklist = v
                .split(',')
                .filter(|s| !s.is_empty())
                .map(|s| s.trim().to_uppercase())
...
pub async fn run(
    ...
    live_exec: Option<Arc<hl_pairs::exec::LiveExec>>,
    live_state: Option<hl_pairs::live_state::LiveState>,
) {
    info!(
        ...
        "[hl_spread_revert] starting (baseline-driven)"
    );
    ...
    // doesn't miss its earliest entry signals on cold start.
    {
        ...
        if n == 0 {
            warn!("[hl_spread_revert] baselines still empty after 30 s — entries will idle until populated");
        } else {
            info!(n_bases = n, "[hl_spread_revert] baselines ready");
        }
    }
    ...
    // Background sweeper for time-based exits — fires every 200 ms.
    {
        ...
        let live_exec_sweep = live_exec.clone();
        let live_state_sweep = live_state.clone();
        tokio::spawn(async move {
            let mut iv = tokio::time::interval(StdDuration::from_millis(200));
            loop {
                iv.tick().await;
                sweep_exits(&state, &writer, &params, &baselines, &live_exec_sweep, &live_state_sweep).await;
            }
        });
    }
    ...
    loop {
        match tick_rx.recv().await {
            Ok(t) => {
                if let Err(e) = on_tick(&state, &writer, &t, &params, &baselines, &live_exec, &live_state).await {
                    warn!(error = %e, "[hl_spread_revert] on_tick err");
                }
            }
            Err(broadcast::error::RecvError::Lagged(n)) => {
                warn!(missed = n, "[hl_spread_revert] tick lagged");
            }
            Err(_) => break,
        }
    }
...
async fn on_tick(
    ...
    for o in open_acts {
        ...
        // ---- LIVE: entry IOC + immediate maker exit limit at TP ----
        if let Some(le) = &live_exec {
            fire_live_entry_with_maker_exit(
                le.clone(), o.base.clone(), o.side_s, o.entry_price, o.size_usd, o.pos_id, &params.account_id,
                live_state,
            );
        }
        if let Err(e) = writer
            ...
            .await
        {
            warn!(error = %e, "[hl_spread_revert] entry signal write failed");
        }
    }
    for c in close_acts {
        log_close(c, writer, params, &live_exec, &live_state).await;
    }
    Ok(())
}
...
async fn log_close(c: CloseAction, writer: &IlpWriter, params: &SpreadRevertParams, live_exec: &Option<Arc<hl_pairs::exec::LiveExec>>, _live_state: &Option<hl_pairs::live_state::LiveState>) {
    ...
    info!(
        ...
        "[hl_spread_revert] CLOSE"
    );

    let side_s = if c.pos.side == 1 { "long" } else { "short" };
    // ---- LIVE: cancel pending maker exit + IOC fallback if not yet filled ----
    if let Some(le) = live_exec {
        fire_live_close(le.clone(), c.pos.id, c.exit_price, &params.account_id);
        // Track realized PnL into kill switch (paper estimate)
        let pnl_bp_for_track = pnl_bp_gross - 2.0 * params.flipster_fee_bp;
        let pnl_usd = params.entry_size_usd * pnl_bp_for_track / 1e4;
    ...
    if let Err(e) = writer
        ...
        .await
    {
        warn!(error = %e, "[hl_spread_revert] position_log write failed");
    }
    ...
    if let Err(e) = writer
        ...
        .await
    {
        warn!(error = %e, "[hl_spread_revert] exit signal write failed");
    }
}
