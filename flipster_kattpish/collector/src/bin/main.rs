...
async fn main() -> Result<()> {
    ...
    // 65k slots handles ~10s of burst at expanded symbol counts.
    let (tx, _rx) = broadcast::channel::<BookTick>(65_536);
        ...
        .unwrap_or(false)
    {
        ...
        for params in variants {
            tracing::info!(variant = %params.account_id, "paper_bot variant enabled");
