...
//! bot, writing to `position_log` with `strategy="bingx_lead"`. It also emits
...
use pairs_core::{base_of as pairs_core_base_of, single_leg_pnl_bp, ExecutorEvent};
...
pub struct GateLeadParams {
    ...
    pub mode_tag: String,
    /// Follower exchange whose ticks update flipster_bid/ask state and
    /// trigger TP/stop checks. Default Bingx (legacy). Set to Lighter
    /// to run the same lead-lag detection against the Lighter universe
    /// — Lighter has 0/0 fees so smaller alpha thresholds (lower
    /// min_move_bp + exit_bp) become viable.
    pub follower_venue: ExchangeName,
}

impl Default for GateLeadParams {
    fn default() -> Self {
        Self {
            account_id: "BINGX_LEAD_v1".to_string(),
            size_usd: 5.0,
            min_move_bp: 20.0,
            anchor_s: 3.0,
            hold_max_s: 2.0,
            ...
            stop_bp: 8.0,
            cooldown_s: 3.0,
            // BingX taker effective: raw 5 bp × Banner 32% paid (68% off) =
            // 1.6 bp/side. Round-trip = 3.2 bp. Earlier 0.85 default was
            // inherited from gate_lead (Flipster-tuned) and underestimated
            // BingX fees by ~1.5 bp per trade.
            fee_bp_per_side: 1.6,
            backtest_mode: false,
            ...
            follower_venue: ExchangeName::Bingx,
        }
    }
}
...
impl GateLeadParams {
    pub fn from_env() -> Self {
        ...
        if let Ok(v) = std::env::var("GL_FOLLOWER_VENUE") {
            p.follower_venue = match v.to_lowercase().as_str() {
                "bingx" => ExchangeName::Bingx,
                "lighter" => ExchangeName::Lighter,
                "aster" => ExchangeName::Aster,
                "pancake" => ExchangeName::Pancake,
                "flipster" => ExchangeName::Flipster,
                "gate" => ExchangeName::Gate,
                "mexc" => ExchangeName::Mexc,
                other => {
                    tracing::warn!(value = %other, "unknown GL_FOLLOWER_VENUE — keeping Bingx");
                    ExchangeName::Bingx
                }
            };
        }
...
pub async fn run(
    ...
    fill_rx: Option<broadcast::Receiver<ExecutorEvent>>,
    ...
    // Fill feedback consumer — retunes ref_mid + entry_price on the open
    // position when the executor confirms a real fill. Without this the
    // strategy's TP threshold is computed against signal-time mid, which
    // ignores the half-spread head start that maker entry already captured
    // — leading to "we're up 15bp but TP at +7bp doesn't fire" symptoms.
    if let Some(mut rx) = fill_rx {
        let state = state.clone();
        let account_id = params.account_id.clone();
        tokio::spawn(async move {
            loop {
                match rx.recv().await {
                    Ok(ExecutorEvent::Fill(f)) if f.account_id == account_id
                        && f.action == "entry"
                        && f.flipster_price > 0.0 =>
                    {
                        let base = f.base.clone();
                        let new_ref = f.flipster_price;
                        let mut s = state.lock().await;
                        if let Some(bs) = s.get_mut(&base) {
                            if let Some(pos) = bs.open.as_mut() {
                                let prev = pos.ref_mid;
                                pos.ref_mid = new_ref;
                                pos.entry_price = new_ref;
                                info!(
                                    base = %base,
                                    prev_ref_mid = prev,
                                    new_ref_mid = new_ref,
                                    delta_bp = format!("{:+.2}",
                                        (new_ref - prev) / prev.abs().max(1e-12) * 1e4),
                                    "[bingx_lead] ref_mid retuned to real fill"
                                );
                            }
                        }
                    }
                    Ok(_) => {} // ignore exits + non-matching events
                    Err(broadcast::error::RecvError::Lagged(n)) => {
                        warn!(missed = n, "[bingx_lead] fill_rx lagged");
                    }
                    Err(_) => break,
                }
            }
        ...
        match tick_rx.recv().await {
            Ok(t) => {
                if let Err(e) = on_tick(&state, &writer, &t, &params).await {
                    warn!(error = %e, "[bingx_lead] on_tick err");
                }
            }
            Err(broadcast::error::RecvError::Lagged(n)) => {
                warn!(missed = n, "[bingx_lead] tick lagged");
            }
            Err(_) => break,
        }
    }
...
async fn on_tick(
    ...
    let mut close_act: Option<CloseAction> = None;
    {
        ...
        match tick.exchange {
            ...
            ExchangeName::Binance => {
                ...
                if let Some(stop_at) = entry.last_stop_at {
                    ...
                    if elapsed_s < stop_cd_s {
                        info!(
                            ...
                            stop_cd_s,
                            "[bingx_lead] SKIP — whipsaw stop-cooldown"
                        );
                        return Ok(());
                    }
                }
                ...
                if lag_bp < min_lag_bp {
                    info!(
                        ...
                        min_lag_bp,
                        "[bingx_lead] SKIP — Flipster already ahead of Binance"
                    );
                    return Ok(());
                }
                    ...
                    if fa > 0.0 {
                        ...
                        if flip_move_bp > max_vel_bp {
                            info!(
                                ...
                                max_vel_bp,
                                vel_window_ms,
                                "[bingx_lead] SKIP — Flipster already catching up"
                            );
                            return Ok(());
                        }
                    }
                ...
                if bin_retrace_bp > 0.0 {
                    ...
                    if retrace > bin_retrace_bp {
                        info!(
                            ...
                            bin_retrace_bp,
                            "[bingx_lead] SKIP — BIN already retraced from peak"
                        );
                        return Ok(());
                    }
                }
                ...
                if pre_revert_bp > 0.0 {
                    ...
                    if prior_at_current {
                        info!(
                            ...
                            pre_window_ms,
                            pre_revert_bp,
                            "[bingx_lead] SKIP — BIN was at current level before anchor (round-trip)"
                        );
                        return Ok(());
                    }
                }
            ...
            // Follower exchange updates the latest quote + may trigger exit.
            e if e == params.follower_venue => {
    ...
    if let Some(o) = open_act {
        ...
        if let Err(e) = writer
            ...
            .await
        {
            warn!(error = %e, "[bingx_lead] entry signal write failed");
        }
    }
...
async fn log_close(c: CloseAction, writer: &IlpWriter, params: &GateLeadParams) {
    ...
    if let Err(e) = writer
        ...
        .await
    {
        warn!(error = %e, "[bingx_lead] position_log write failed");
    }
    ...
    if let Err(e) = writer
        ...
        .await
    {
        warn!(error = %e, "[bingx_lead] exit signal write failed");
    }
}
