...
//! bot, writing to `position_log` with `strategy="mexc_lead"`. It also emits
...
impl Default for GateLeadParams {
    fn default() -> Self {
        Self {
            account_id: "MEXC_LEAD_v1".to_string(),
            size_usd: 5.0,
            min_move_bp: 20.0,
            anchor_s: 3.0,
            hold_max_s: 2.0,
            ...
            stop_bp: 8.0,
            cooldown_s: 3.0,
            // MEXC perp taker = 0.02% = 2 bp/side (no Banner payback applied,
            // banner shows 35% rebate but cer.live + manual confirmation
            // says it doesn't apply to perp futures). Round-trip = 4 bp.
            fee_bp_per_side: 2.0,
            backtest_mode: false,
        ...
        match tick_rx.recv().await {
            Ok(t) => {
                if let Err(e) = on_tick(&state, &writer, &t, &params).await {
                    warn!(error = %e, "[mexc_lead] on_tick err");
                }
            }
            Err(broadcast::error::RecvError::Lagged(n)) => {
                warn!(missed = n, "[mexc_lead] tick lagged");
            }
            Err(_) => break,
        }
    }
...
async fn on_tick(
    ...
    let mut close_act: Option<CloseAction> = None;
    {
        ...
        match tick.exchange {
            ...
            ExchangeName::Binance => {
                ...
                if let Some(stop_at) = entry.last_stop_at {
                    ...
                    if elapsed_s < stop_cd_s {
                        info!(
                            ...
                            stop_cd_s,
                            "[mexc_lead] SKIP — whipsaw stop-cooldown"
                        );
                        return Ok(());
                    }
                }
                ...
                if lag_bp < min_lag_bp {
                    info!(
                        ...
                        min_lag_bp,
                        "[mexc_lead] SKIP — Flipster already ahead of Binance"
                    );
                    return Ok(());
                }
                    ...
                    if fa > 0.0 {
                        ...
                        if flip_move_bp > max_vel_bp {
                            info!(
                                ...
                                max_vel_bp,
                                vel_window_ms,
                                "[mexc_lead] SKIP — Flipster already catching up"
                            );
                            return Ok(());
                        }
                    }
                ...
                if bin_retrace_bp > 0.0 {
                    ...
                    if retrace > bin_retrace_bp {
                        info!(
                            ...
                            bin_retrace_bp,
                            "[mexc_lead] SKIP — BIN already retraced from peak"
                        );
                        return Ok(());
                    }
                }
                ...
                if pre_revert_bp > 0.0 {
                    ...
                    if prior_at_current {
                        info!(
                            ...
                            pre_window_ms,
                            pre_revert_bp,
                            "[mexc_lead] SKIP — BIN was at current level before anchor (round-trip)"
                        );
                        return Ok(());
                    }
                }
            ...
            e if matches!(e, ExchangeName::Mexc) => {
    ...
    if let Some(o) = open_act {
        ...
        if let Err(e) = writer
            ...
            .await
        {
            warn!(error = %e, "[mexc_lead] entry signal write failed");
        }
    }
...
async fn log_close(c: CloseAction, writer: &IlpWriter, params: &GateLeadParams) {
    ...
    if let Err(e) = writer
        ...
        .await
    {
        warn!(error = %e, "[mexc_lead] position_log write failed");
    }
    ...
    if let Err(e) = writer
        ...
        .await
    {
        warn!(error = %e, "[mexc_lead] exit signal write failed");
    }
}
