...
//! bot, writing to `position_log` with `strategy="hl_lead"`. It also emits
        ...
        match tick_rx.recv().await {
            Ok(t) => {
                if let Err(e) = on_tick(&state, &writer, &t, &params).await {
                    warn!(error = %e, "[hl_lead] on_tick err");
                }
            }
            Err(broadcast::error::RecvError::Lagged(n)) => {
                warn!(missed = n, "[hl_lead] tick lagged");
            }
            Err(_) => break,
        }
    }
...
async fn on_tick(
    ...
    let mut close_act: Option<CloseAction> = None;
    {
        ...
        match tick.exchange {
            ...
            ExchangeName::Binance => {
                ...
                if let Some(stop_at) = entry.last_stop_at {
                    ...
                    if elapsed_s < stop_cd_s {
                        info!(
                            ...
                            stop_cd_s,
                            "[hl_lead] SKIP — whipsaw stop-cooldown"
                        );
                        return Ok(());
                    }
                }
                ...
                if lag_bp < min_lag_bp {
                    info!(
                        ...
                        min_lag_bp,
                        "[hl_lead] SKIP — Flipster already ahead of Binance"
                    );
                    return Ok(());
                }
                    ...
                    if fa > 0.0 {
                        ...
                        if flip_move_bp > max_vel_bp {
                            info!(
                                ...
                                max_vel_bp,
                                vel_window_ms,
                                "[hl_lead] SKIP — Flipster already catching up"
                            );
                            return Ok(());
                        }
                    }
                ...
                if bin_retrace_bp > 0.0 {
                    ...
                    if retrace > bin_retrace_bp {
                        info!(
                            ...
                            bin_retrace_bp,
                            "[hl_lead] SKIP — BIN already retraced from peak"
                        );
                        return Ok(());
                    }
                }
                ...
                if pre_revert_bp > 0.0 {
                    ...
                    if prior_at_current {
                        info!(
                            ...
                            pre_window_ms,
                            pre_revert_bp,
                            "[hl_lead] SKIP — BIN was at current level before anchor (round-trip)"
                        );
                        return Ok(());
                    }
                }
            ...
            e if matches!(e, ExchangeName::Hyperliquid) => {
    ...
    if let Some(o) = open_act {
        ...
        if let Err(e) = writer
            ...
            .await
        {
            warn!(error = %e, "[hl_lead] entry signal write failed");
        }
    }
...
async fn log_close(c: CloseAction, writer: &IlpWriter, params: &GateLeadParams) {
    ...
    if let Err(e) = writer
        ...
        .await
    {
        warn!(error = %e, "[hl_lead] position_log write failed");
    }
    ...
    if let Err(e) = writer
        ...
        .await
    {
        warn!(error = %e, "[hl_lead] exit signal write failed");
    }
}
