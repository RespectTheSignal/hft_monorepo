use crate::{
    ...
    market_watcher, order_manager_client, state_manager, strategy, strategy_core, zmq_sub_client,
};
...
pub fn initialize() -> anyhow::Result<Option<f64>> {
    ...
    // Flipster mode: if FLIPSTER_CDP_PORT is set, initialize the global cookie store
    // synchronously so every downstream check `crate::flipster_cookie_store::global()`
    // reports Some.
    if let Ok(cdp_port_s) = std::env::var("FLIPSTER_CDP_PORT") {
        if let Ok(cdp_port) = cdp_port_s.parse::<u16>() {
            // Build a small one-shot tokio runtime for the cookie fetch.
            ...
            info!("[Flipster Rust] Cookie store initialized via CDP port {}", cdp_port);
        }
    }
...
fn parse_args() -> anyhow::Result<(
    ...
    // Flipster mode default: single "flipster" login (cookie is the credential).
...
pub fn start(core: Arc<dyn strategy_core::StrategyCore>) -> anyhow::Result<()> {
    ...
    let configs = if crate::flipster_cookie_store::global().is_some() {
        // Flipster mode: skip Supabase, build a single config from local JSON +
        // symbols_v9.json. Mirrors the gate_hft schema without requiring DB.
        build_flipster_configs(&login_names)?
    } else {
        loop {
    ...
    // Load account credentials — in Flipster mode, bootstrap an in-memory AccountManager
    // with a single dummy entry (cookie auth supersedes api-key/secret).
    let account_manager = if crate::flipster_cookie_store::global().is_some() {
        write_flipster_dummy_account_files()?;
    ...
    let data_client: Arc<dyn DataClient> = if crate::flipster_cookie_store::global().is_some() {
        // Flipster mode: connect api+web WS directly.
        let api_key = std::env::var("FLIPSTER_API_KEY")
            .unwrap_or_else(|_| {
                warn!("FLIPSTER_API_KEY not set");
                String::new()
            });
        ...
        let cdp_port = crate::flipster_cookie_store::global()
            ...
            .unwrap_or(9230);
        info!(
            "[Flipster Rust] WS data client: cdp_port={}, symbols={}",
            cdp_port,
            initial_symbols.len()
        );
    ...
    std::thread::spawn(move || {
        ...
        loop {
            ...
            let overrides_by_login: std::collections::HashMap<
                String,
                config::TradingRuntimeOverrides,
            > = new_configs
                .iter()
                .map(|c| {
                    (
    ...
    let use_market_watcher = std::env::var("USE_MARKET_WATCHER")
        ...
        .unwrap_or(false);
    if !use_market_watcher && core.name() == "V10Strategy" {
        bail!("[GateHFT Rust] V10 requires market watcher (USE_MARKET_WATCHER=false)");
    }
    ...
    if use_market_watcher {
        ...
        // Flipster mode (cookie store initialized) uses flipster_bookticker table as
        // the trading venue; otherwise keep gate_hft legacy "gate" table.
    ...
    let strategy = strategy::Strategy::new(
...
// =========================================================================
// Flipster mode bootstrap: build StrategyConfig locally (no Supabase) and
// write dummy credential files so AccountManager::new succeeds.
// =========================================================================

pub fn build_flipster_configs_pub(login_names: &[String]) -> anyhow::Result<Vec<config::StrategyConfig>> {
    build_flipster_configs(login_names)
}

pub fn write_flipster_dummy_account_files_pub() -> anyhow::Result<()> {
    write_flipster_dummy_account_files()
}

fn write_flipster_dummy_account_files() -> anyhow::Result<()> {
    use std::fs;

    // sub_account_keys: object keyed arbitrarily; each entry must match SubAccountKey shape
    // (account_manager::load_sub_account_keys iterates as_object()).
    // ip_whitelist must contain current_ip ("127.0.0.1") or the entry is skipped.
    let sub = r#"{
        "flipster": {
            "state": 1,
            "mode": 0,
    ...
    // main_account_secrets: HashMap<i32, MainAccountSecret> — keyed by uid as string.
    let main = r#"{
        "0": {
            "api_key": "-",
            "secret_key": "-",
    ...
    fs::write("/tmp/flipster_subaccount_keys.json", sub)?;
    fs::write("/tmp/flipster_main_account_secrets.json", main)?;
    Ok(())
}

fn build_flipster_configs(login_names: &[String]) -> anyhow::Result<Vec<config::StrategyConfig>> {
    // 1. TradeSettings: load from JSON (env FLIPSTER_TRADE_SETTINGS_JSON, inline or file path).
    let ts_text = match std::env::var("FLIPSTER_TRADE_SETTINGS_JSON") {
        Ok(v) if !v.trim().is_empty() => {
            if v.trim().starts_with('{') {
                v
            } else {
                std::fs::read_to_string(&v).with_context(|| format!("read {}", v))?
            }
        }
        _ => {
            warn!("FLIPSTER_TRADE_SETTINGS_JSON not set — using all defaults (empty JSON).");
            "{}".to_string()
        }
    };
    let trade_settings: config::TradeSettings =
        serde_json::from_str(&ts_text).context("parse FLIPSTER_TRADE_SETTINGS_JSON")?;

    // 2. Symbols: from symbols_v9.json. Override via FLIPSTER_SYMBOLS comma list;
    //    "auto" / "all" / empty → load full symbols_v9.json.
    let symbols: Vec<String> = {
        let csv = std::env::var("FLIPSTER_SYMBOLS").unwrap_or_default();
        let csv_trim = csv.trim().to_lowercase();
        let use_all = csv.trim().is_empty() || csv_trim == "auto" || csv_trim == "all";
        if !use_all {
            csv.split(',')
            ...
            let path = crate::flipster_contracts::symbols_v9_path();
            let data = std::fs::read_to_string(&path)
                .with_context(|| format!("read {}", path.display()))?;
            let arr: Vec<serde_json::Value> =
                serde_json::from_str(&data).context("parse symbols_v9.json")?;
            arr.into_iter()
                .filter_map(|v| v.get("base").and_then(|b| b.as_str()).map(|s| format!("{}USDT.PERP", s)))
                .collect()
        }
    };
    info!(
        "[Flipster Rust] local config: login_names={:?}, {} symbols, order_size={}",
        login_names,
        symbols.len(),
        trade_settings.order_size
    );

    // 3. Build one StrategyConfig per login_name (typically just "flipster").
    let configs = login_names
        .iter()
        .map(|name| {
            config::StrategyConfig::new(name.clone(), symbols.clone(), trade_settings.clone(), None)
        })
        .collect();
    Ok(configs)
}
